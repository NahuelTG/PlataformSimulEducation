...
import MisCursos from "../../assets/MisCursos.png";
                              ...
                              {messages.map((mensaje) => (
                                 <div
                                    ...
                                    className={`${styles.message} ${
                                       mensaje.senderEmail === currentUser.email ? styles.sent : styles.received
                                    }`}
                                 >
                     ...
                     <div>
                        <div className={styles.mensajestabs}>
                           <button
                              ...
                              className={activeTab === "mensajes" ? styles.mensajeactive : ""}
                           >
                              Mensajes
                           </button>
                           <button
                              onClick={() => setActiveTab("usuarios")}
                              className={activeTab === "usuarios" ? styles.mensajeactive : ""}
                           >
                              Usuarios
                           </button>
                           <button onClick={() => setActiveTab("admin")} className={activeTab === "admin" ? styles.mensajeactive : ""}>
                              Admin
                           </button>
                        </div>
                        <div className={styles.mensajescontent}>
                           ...
                           <div
                              ...
                              style={{ display: activeTab === "usuarios" || activeTab === "admin" ? "block" : "none" }}
                           >
                              <h2>{activeTab === "usuarios" ? "Usuarios" : "Administradores"}</h2>
                              {contacts
                                 .filter((contact) => contact.role === (activeTab === "usuarios" ? "usuario" : "admin"))
                                 .map((contact) => (
                                    <div key={contact.email} className={styles.contactitem} onClick={() => handleContactClick(contact)}>
                                       <p>{contact.username}</p>
                                    </div>
      ...
      <nav className={styles.nav}>
         ...
         {isVisible && menuActive && (
            <ul className={styles.nav__items_mobile}>
               <NavLink className={({ isActive }) => `${isActive ? styles.active : ""} ${styles.nav__item}`} to="/">
                  <img src={Home} alt="" />
                  <span className={styles.label}>Inicio</span>
               </NavLink>
               <NavLink className={({ isActive }) => `${isActive ? styles.active : ""} ${styles.nav__item}`} to="/User/MyCourses">
                  <span className={styles.label}>Mis Cursos</span>
                  <FontAwesomeIcon icon={faGraduationCap} />
               </NavLink>
               <NavLink className={({ isActive }) => `${isActive ? styles.active : ""} ${styles.nav__item}`} to="/User/PrueCod">
                  <span className={styles.label}>Probar Codigo</span>
                  <FontAwesomeIcon icon={faFileCode} />
               </NavLink>
               <NavLink className={({ isActive }) => `${isActive && styles.active} ${styles.nav__item}`} to="/User/VideoCall">
               ...
               <div className={styles.nav__item}>
                  ...
                  <NavLink to="/User/Search" className={({ isActive }) => `${isActive ? styles.active : ""} ${styles.iconsearch}`}>
         ...
         {!isVisible && (
            <ul className={styles.nav__items}>
               <NavLink className={() => `${styles.active} ${styles.nav__item}`} to="/">
                  <img src={Home} alt="" />
                  <span className={styles.label}>Inicio</span>
               </NavLink>
               <NavLink className={() => `${styles.active} ${styles.nav__item}`} to="/User/MyCourses">
                  <img src={MisCursos} alt="" />
                  <span className={styles.label}>Mis Cursos</span>
               </NavLink>
               <NavLink className={() => `${styles.active} ${styles.nav__item}`} to="/User/PrueCod">
                  <img src={Codigo} alt="" />
                  <span className={styles.label}>
                     Editor de <br /> Codigo
                  </span>
               </NavLink>
               <NavLink className={() => `${styles.active} ${styles.nav__item}`} to="/User/VideoCall">
